...
namespace JsonExSerializer.MetaData
{
    ...
    public class CustomTypeDataRepository : TypeDataRepository
    {
        ...
        public CustomTypeDataRepository(Type TypeHandlerType, ISerializerSettings config)
            : base(config)
        {
        ...
        protected override TypeData CreateNew(Type forType)
        {
            ...
            for (int i = 0; i < parms.Length; i++)
            {
                if (parms[i].ParameterType.IsAssignableFrom(typeof(Type)))
                    args[i] = forType;
                else if (parms[i].ParameterType.IsAssignableFrom(typeof(ISerializerSettings)))
                    args[i] = this.Config;
            }
                ...
                foreach (ConstructorInfo constructor in ctors)
                {
                    ...
                    foreach (ParameterInfo p in parms)
                    {
                        if (p.ParameterType != typeof(Type)
                            && !p.ParameterType.IsAssignableFrom(typeof(ISerializerSettings)))
                        {
